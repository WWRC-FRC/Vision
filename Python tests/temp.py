...
#	contours = cv2.findContours(gray_bi_image, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
	gray_bi_image_temp = gray_bi_image
	contours = cv2.findContours(gray_bi_image_temp, 1, 2)

	edged_bi_image_temp = edged_bi_image
	contours,hierarchy = cv2.findContours(edged_bi_image_temp, 1, 2)
	cnt = contours[0]
	M = cv2.moments(cnt)
#	print M
	cx = int(M['m10']/M['m00'])
...
#	cropped_image = region_of_interest( cannyed_image, np.array([region_of_interest_vertices], np.int32))
	...
	#HaughLinesP version
	edged_bi_image_temp = edged_bi_image
	plines_bi = cv2.HoughLinesP( edged_bi_image_temp, rho=HRho, theta= HTheta, threshold=HThresh, lines=np.array([]), minLineLength=HLen, maxLineGap=HGap)
	...
	#HaughLines version
	edged_bi_image_temp = edged_bi_image
	lines_bi = cv2.HoughLines( edged_bi_image_temp,HRho, HTheta, HThresh)
